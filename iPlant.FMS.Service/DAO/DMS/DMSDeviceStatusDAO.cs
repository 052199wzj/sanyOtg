    ...
    class DMSDeviceStatusDAO : BaseDAO
    {
                ...
                OutResult<Int32> wErrorCode)
       {
            ...
            try
            {
                ...
                String wSQL = StringUtils.Format(
                        "SELECT p.ID,t.ID AS DeviceIDL,p.*,t.Code as DeviceNo,t.Name as DeviceName,t.ImageIcon, t.ModelID,t.AssetNo,t.Remark," +
                        " t.WorkShopID,t.FactoryID,t.LineID,t.AreaID,t.TeamID, t8.Name as TeamName, t8.Code as TeamNo," +
                        " t1.Code as  ModelNo,t1.Name as ModelName,t1.DeviceType, t2.Name as DeviceTypeName," +
                        "  t2.Code as DeviceTypeCode,t3.Name as WorkShopName,t3.Code as WorkShopCode,t7.Code as AreaNo," +
                        " t7.Name as PositionText,t9.Name as FactoryName,t9.Code as FactoryCode, count(a1.DeviceID) as AlarmCount,    "+
                        " count(q.StartTime>=CURDATE())  as NumTotal, "
                                ...
                                + " left join {0}.dms_device_processrecord q on q.AssetNo = t.AssetNo"
                                + " left join {0}.fmc_line t4 on t.LineID=t4.ID  WHERE  1=1  "
                ...
                foreach (Dictionary<String, Object> wReader in wQueryResult)
                {
                    ...
                    wDeviceStatus.NumTotal = StringUtils.parseInt(wReader["NumTotal"]);

                    wResult.Add(wDeviceStatus);
                }
                Dictionary<string, List<DMSDeviceStatus>> wRecordIDList = wResult.GroupBy(p => p.AssetNo).ToDictionary(p => p.Key, p => p.ToList());
                string wSql1 = StringUtils.Format("select p.ParameterValue,p.DeviceID,p1.VariableName From {0}.dms_device_parametervalue p left join " +
                    " {0}.dms_device_parameter p1 on p.ParameterNo=p1.Code   where p.DeviceID IN ({1})", wInstance, StringUtils.Join(",", wRecordIDList.Keys));
                List<Dictionary<String, Object>> wQueryResult1 = mDBPool.queryForList(wSql1, null);

                Dictionary<string, Object> qReader = new Dictionary<string, Object>();

                string qAseetNo = "";
                foreach (Dictionary<String, Object> wReader in wQueryResult1)
                {
                    qReader.Clear();
                    qAseetNo = StringUtils.parseString(wReader["DeviceID"]);
                    if (qReader.ContainsKey(StringUtils.parseString(wReader["VariableName"])))
                        continue;
                    qReader.Add(StringUtils.parseString(wReader["VariableName"]),StringUtils.parseString(wReader["ParameterValue"]));
                    wRecordIDList[qAseetNo].ForEach(p => p.RealTimeParams.AddRange(qReader));
            ...
            OutResult<Int32> wErrorCode)
        {
            ...
            try
            {
                ...
                if (wDeviceID > 0 || StringUtils.isNotEmpty(wDeviceNo) || StringUtils.isNotEmpty(wAssetNo))
                    ...
                    wAssetNo, -1, -1, -1, -1, -1, -1, -1, -1, -1,wErrorCode);
