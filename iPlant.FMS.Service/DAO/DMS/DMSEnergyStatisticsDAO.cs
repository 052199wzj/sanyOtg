...
namespace iPlant.FMC.Service
{
      ...
      int wActive, OutResult<Int32> wErrorCode)
        {
            ...
            try
            {
                ...
                wParamMap.Add("wEnergyType", wEnergyType);
                wParamMap.Add("wStartTime", wStartTime);
                wParamMap.Add("wEndTime", wEndTime.AddDays(1));
                wParamMap.Add("wDeviceID", StringUtils.Join(",", wDeviceIDList));
                wParamMap.Add("wActive", wActive);
                ...
                foreach (Dictionary<String, Object> wReader in wQueryResult)
                {
                    ...
                    wDeviceModelW.DeviceNo = StringUtils.parseString(wReader["DeviceNo"]);
                    wDeviceModelW.AssetNo = StringUtils.parseString(wReader["AssetNo"]);
                    wDeviceModelW.StatType = ((int)DMSStatTypes.Hour);
                    wDeviceModelW.EnergyType = StringUtils.parseInt(wReader["EnergyType"]);
                    wDeviceModelW.StatConsumption = StringUtils.parseDouble(wReader["StatConsumption"]);
                    wDeviceModelW.UpdateTime = StringUtils.parseDate(wReader["UpdateTime"]);
                    wDeviceModelW.StatStartDate = StringUtils.parseDate(wReader["StatStartDate"]);
  ...
  //      }
	 ...
	 String wInstance =iPlant.Data.EF.MESDBSource.DMS.getDBName();
                if (wErrorCode.Result != 0)
                    return wResult;
                String wSQL = StringUtils.Format("SELECT t.DeviceID,t.UpdateTime,t.EnergyType, str_to_date( DATE_FORMAT(t.StatStartDate,'%Y-%m-1'),'%Y-%m-%d') as MonthDate," +
                    " sum(t.StatConsumption) as SumConsumption  ,  t1.Code as DeviceNo,t1.Name as DeviceName,t1.AssetNo FROM {0}.dms_energy_stat t" +
                    " inner join {0}.dms_device_ledger  t1 on t.DeviceID=t1.ID WHERE t.StatType  = @wStatType and  t.Active=1  "
                        + " and ( @wAreaID <= 0 or t1.AreaID  = @wAreaID) "
                        + " and ( @wEnergyType ='' or t.EnergyType  = @wEnergyType)  "
                        + " and  str_to_date( DATE_FORMAT( @wStartTime,'%Y-1-1'),'%Y-%m-%d') <=t.StatStartDate "
                        + " and  str_to_date( DATE_FORMAT(@wEndTime,'%Y-1-1'),'%Y-%m-%d') >=t.StatStartDate "
                        + " and ( @wDeviceID ='' or t.DeviceID IN( {1} ) ) " +
                        " group by t.DeviceID,t.EnergyType,MonthDate; ", wInstance, wDeviceIDList.Count > 0 ? StringUtils.Join(",", wDeviceIDList) : "0");
                wSQL = this.DMLChange(wSQL);
                Dictionary<String, Object> wParamMap = new Dictionary<String, Object>();
                ...
                wParamMap.Add("wEnergyType", wEnergyType);
                wParamMap.Add("wStartTime", wStartTime);
                wParamMap.Add("wEndTime", wEndTime.AddYears(1));
                wParamMap.Add("wDeviceID", StringUtils.Join(",", wDeviceIDList));
                ...
                foreach (Dictionary<String, Object> wReader in wQueryResult)
                {
                    ...
                    wDeviceModelW.DeviceNo = StringUtils.parseString(wReader["DeviceNo"]);
                    wDeviceModelW.AssetNo = StringUtils.parseString(wReader["AssetNo"]);
                    wDeviceModelW.StatType = ((int)DMSStatTypes.Month);
                    wDeviceModelW.EnergyType = StringUtils.parseInt(wReader["EnergyType"]);
                    wDeviceModelW.StatConsumption = StringUtils.parseDouble(wReader["SumConsumption"]);
                    wDeviceModelW.UpdateTime = StringUtils.parseDate(wReader["UpdateTime"]);
                    wDeviceModelW.StatStartDate = StringUtils.parseDate(wReader["MonthDate"]);
        ...
        /// <param name="wDeviceIDList"></param>
        /// <param name="wAreaID"></param>
        /// <param name="wEnergyType"></param>
        /// <param name="wStartTime"></param>
        /// <param name="wEndTime"></param>
        /// <param name="wErrorCode"></param>
        /// <returns></returns>
        public List<DMSEnergyStatistics> DMS_SelectEnergyStatisticsDayList(BMSEmployee wLoginUser, List<int> wDeviceIDList,
        int wAreaID, int wEnergyType, DateTime wStartTime, DateTime wEndTime,  OutResult<Int32> wErrorCode)
        {
            List<DMSEnergyStatistics> wResult = new List<DMSEnergyStatistics>();
            try
            {
                ...
                if (wErrorCode.Result != 0)
                    return wResult;
                String wSQL = StringUtils.Format("SELECT t.DeviceID,t.UpdateTime,t.EnergyType, str_to_date( DATE_FORMAT(t.StatStartDate,'%Y-%m-%d'),'%Y-%m-%d') as DayDate," +
                    " sum(t.StatConsumption) as SumConsumption  ,  t1.Code as DeviceNo,t1.Name as DeviceName,t1.AssetNo FROM {0}.dms_energy_stat t" +
                    " inner join {0}.dms_device_ledger  t1 on t.DeviceID=t1.ID WHERE t.StatType  = @wStatType and  t.Active=1  "
                        + " and ( @wAreaID <= 0 or t1.AreaID  = @wAreaID) "
                        + " and ( @wEnergyType ='' or t.EnergyType  = @wEnergyType)  "
                        + " and  str_to_date( DATE_FORMAT(@wStartTime,'%Y-%m-1'),'%Y-%m-%d') <=t.StatStartDate  "
                        + " and  str_to_date( DATE_FORMAT(@wEndTime,'%Y-%m-1'),'%Y-%m-%d') >=t.StatStartDate  "
                        + " and ( @wDeviceID ='' or t.DeviceID IN( {1} ) ) " +
                        " group by t.DeviceID,t.EnergyType,DayDate; ", wInstance, wDeviceIDList.Count > 0 ? StringUtils.Join(",", wDeviceIDList) : "0");
                wSQL = this.DMLChange(wSQL);
                Dictionary<String, Object> wParamMap = new Dictionary<String, Object>();
                ...
                wParamMap.Add("wEnergyType", wEnergyType);
                wParamMap.Add("wStartTime", wStartTime);
                wParamMap.Add("wEndTime", wEndTime.AddMonths(1));
                wParamMap.Add("wDeviceID", StringUtils.Join(",", wDeviceIDList));
                ...
                foreach (Dictionary<String, Object> wReader in wQueryResult)
                {
                    ...
                    wDeviceModelW.StatConsumption = StringUtils.parseDouble(wReader["SumConsumption"]);
                    wDeviceModelW.UpdateTime = StringUtils.parseDate(wReader["UpdateTime"]);
                    wDeviceModelW.StatStartDate = StringUtils.parseDate(wReader["DayDate"]);
                    wDeviceModelW.StatEndDate = wDeviceModelW.StatStartDate.AddDays(1).AddMilliseconds(-1);
                    wDeviceModelW.Active = 1;
            ...
            try
            {
                switch (wStatType)
                {
                    case ((int)DMSStatTypes.Week):
                        ...
                        break;
                    case ((int)DMSStatTypes.Month):
                        wResult = this.DMS_SelectEnergyStatisticsDayList(wLoginUser, wDeviceIDList,
                            wAreaID, wEnergyType, wStartTime, wEndTime,
                             wErrorCode);
                        break;
                    case ((int)DMSStatTypes.Quarter):
