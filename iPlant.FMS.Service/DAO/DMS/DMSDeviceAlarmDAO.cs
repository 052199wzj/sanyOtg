...
namespace iPlant.FMC.Service
{
    class DMSDeviceAlarmDAO : BaseDAO
    {
        ...
        // 10.8  wzj 新增关联表 dms_device_repair 新增查询字段 AlarmLevel 报警等级 ,新加查询时间间隔
               ...
               int wEventType, DateTime wStartTime, DateTime wEndTime, Pagination wPagination, OutResult<Int32> wErrorCode)
        {
            ...
            try
            {
                ...
                "SELECT p.*,timestampdiff(minute,p.UpdateDate,p.OccurTime) as AlarmDuration,t.ID AS DeviceIDL, t.Code as DeviceNo,t.Name as DeviceName,t.ModelID,t.AssetNo," +
        ...
        , wInstance, wDeviceIDList.Count > 0 ? StringUtils.Join(",", wDeviceIDList) : "0");
                ...
                foreach (Dictionary<String, Object> wReader in wQueryResult)
                {
                    ...
                    wDeviceAlarm.AlarmDuration = StringUtils.parseDouble(wReader["AlarmDuration"]);
        ...
        // 10.9  wzj 新增关联表 dms_device_repair 新增查询字段 AlarmLevel 报警等级 
                ...
                OutResult<Int32> wErrorCode)
        {
            ...
            try
            {
                wResult = DMS_SelectDeviceAlarmList(wLoginUser, null, "", wName, wAssetNo, wDeviceType,
                    wModelID, wFactoryID, wWorkShopID, wLineID, wAreaID, wPositionID, wTeamID, wDeviceID, wEventType, wStartTime, wEndTime, wPagination,
                        wErrorCode);
            }
            catch (Exception e)
            {
            ...
            try
            {
                if (wID > 0)
                {
                    ...
                    wResult = this.DMS_SelectDeviceAlarmList(wLoginUser, wIDList, "", "",
                     "", -1, -1, -1, -1, -1, -1 ,- 1, -1, -1,wEventType, wStartTime, wEndTime, wPagination, wErrorCode);
                }
                ...
                for (int i = 0; i < wIDList.Count; i++)
                {
                    ...
                    if (i % 25 == 0)
                    {
                        wResult.AddRange(this.DMS_SelectDeviceAlarmList(wLoginUser, wSelectList, "", "", "", -1, -1, -1, -1, -1,
                            -1, -1, -1,-1, wEventType, wStartTime, wEndTime, wPagination, wErrorCode));
                    ...
                    if (i == wIDList.Count - 1)
                    {
                        if (wSelectList.Count > 0)
                            wResult.AddRange(this.DMS_SelectDeviceAlarmList(wLoginUser, wSelectList, "", "", "", -1, -1, -1, -1, -1, -1, -1, -1,-1,
        ...
        public String DMS_CloseAlarmCodeHistoryString(String wAssetNo)
        {
            //添加EventType=2
