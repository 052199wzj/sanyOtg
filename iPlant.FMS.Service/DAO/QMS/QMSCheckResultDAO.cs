    ...
    class QMSCheckResultDAO : BaseDAO
    {
        ...
        public void QMS_UpdateCheckResult(BMSEmployee wLoginUser, List<QMSCheckResult> wQMSCheckResultList, OutResult<Int32> wErrorCode)
        {
            ...
            try
            {
                ...
                Dictionary<int, List<String>> wParameterNameDic = wQMSCheckResultList.GroupBy(p => p.WorkpieceID).ToDictionary(p => p.Key, p => p.Select(q => q.ParameterName).Distinct().ToList());
                ...
                {
                    wSQLList.Add(StringUtils.Format("DELETE FROM {0}.qms_checkresult WHERE WorkpieceID={1} AND ParameterName IN ('{2}') ;", wInstance, wWorkpieceID, StringUtils.Join("','", wParameterNameDic[wWorkpieceID])));
                }

                String wSql = StringUtils.Format("insert  into   {0}.qms_checkresult  (WorkpieceID,ParameterName,ParameterValue,ParameterDescription,DataType) values ", wInstance);
                ...
                foreach (QMSCheckResult wQMSCheckResult in wQMSCheckResultList)
                {
                    ...
                    if (wQMSCheckResult == null || wQMSCheckResult.WorkpieceID <= 0 || StringUtils.isEmpty(wQMSCheckResult.ParameterName))
                        continue;

                    if (!wParameterNameDic.ContainsKey(wQMSCheckResult.WorkpieceID) || !wParameterNameDic[wQMSCheckResult.WorkpieceID].Contains(wQMSCheckResult.ParameterName))
                        continue;

                    if (wInsertString == null)
                        wInsertString = new StringBuilder(StringUtils.Format("({0},'{1}','{2}','{3}',{4} )", wQMSCheckResult.WorkpieceID,
                        wQMSCheckResult.ParameterName, wQMSCheckResult.ParameterValue, wQMSCheckResult.ParameterDescription, wQMSCheckResult.DataType));
                    else
                        wInsertString.Append(StringUtils.Format(",({0},'{1}','{2}','{3}',{4} )", wQMSCheckResult.WorkpieceID,
                             wQMSCheckResult.ParameterName, wQMSCheckResult.ParameterValue, wQMSCheckResult.ParameterDescription, wQMSCheckResult.DataType));
                ...
                DateTime wStartTime, DateTime wEndTime, Pagination wPagination, OutResult<Int32> wErrorCode)
        {
            ...
            try
            {
                ...
                string wSQL = string.Format(" select t1.*,t.WorkpieceNo FROM {0}.qms_checkresult t1  "
